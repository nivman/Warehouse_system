        ...
        <div class="col-lg-12">
            ...
            <div class="well well-sm">
                <div class="row bold">
                    ...
                    <div class="col-xs-4"><?=lang("date");?> : <?=$this->sma->hrld($inv->date);?><br><?=$inv->reference_no;?> : <?=lang("ref");?></div>
                ...
                <div class="col-xs-5"  style="text-align: right;">
                    ...
                    <?php
                        ...
                        if ($supplier->vat_no != "-" && $supplier->vat_no != "") {
                            echo "<br>" . $supplier->vat_no ." : ". lang("vat_no");
                        }
                        if ($supplier->cf1 != "-" && $supplier->cf1 != "") {

                            echo "<br>" .$supplier->cf1 . " : ". lang("scf1");
                        }
                        if ($supplier->cf2 != "-" && $supplier->cf2 != "") {

                            echo "<br>" .$supplier->cf2 . " : ". lang("scf2");
                        }
                        if ($supplier->cf3 != "-" && $supplier->cf3 != "") {
                            echo "<br>" .$supplier->cf3 . " : ". lang("scf3");
                        }
                        if ($supplier->cf4 != "-" && $supplier->cf4 != "") {
                            echo "<br>" .$supplier->cf4 . " : ". lang("scf4");
                        }
                        if ($supplier->cf5 != "-" && $supplier->cf5 != "") {
                            echo "<br>" .$supplier->cf5 . " : ". lang("scf5");
                        }
                        if ($supplier->cf6 != "-" && $supplier->cf6 != "") {
                            echo "<br>" .$supplier->cf6 . " : ". lang("scf6");
                        }
                        echo "</p>";
                        echo lang("tel") . ": " . $supplier->phone . "<br />" . $supplier->email . " : ". lang("email");
                    ?>
                    <div class="clearfix"></div>
                </div>
                ...
                <table class="table table-bordered table-hover table-striped">
                    ...
                    <?php
                        ...
                        foreach ($rows as $row):
                            ...
                            <tr>
                                ...
                                <?php
                                    if ($Settings->product_discount && $inv->product_discount != 0) {
                                        echo '<td style="width: 100px; text-align:left; vertical-align:middle;">' . ($row->discount != 0 ? '<small>(' . $row->discount . ')</small> ' : '') . $this->sma->formatMoney($row->item_discount) . '</td>';
                                    }
                                ?>
                    ...
                    <?php if ($inv->grand_total != $inv->total) { ?>
                        <tr>
                            <?php
                            if ($Settings->tax1 && $inv->product_tax > 0) {
                                echo '<td style="text-align:left;">' . $this->sma->formatMoney($return_purchase ? ($inv->product_tax+$return_purchase->product_tax) : $inv->product_tax) . '</td>';
                            }
                            if ($Settings->product_discount && $inv->product_discount != 0) {
                                echo '<td style="text-align:left;">' . $this->sma->formatMoney($return_purchase ? ($inv->product_discount+$return_purchase->product_discount) : $inv->product_discount) . '</td>';
                            }
                            ?>
                    ...
                    <?php
                    if ($return_purchase) {
                        echo '<tr><td colspan="' . $col . '" style="text-align:;left;">' . lang("return_total") . ' (' . $default_currency->code . ')</td><td style="text-align:right;">' . $this->sma->formatMoney($return_purchase->grand_total) . '</td></tr>';
                    }
                    if ($inv->surcharge != 0) {
                        echo '<tr>
                                <td colspan="' . $col . '" style="text-align:left;">' . lang("return_surcharge") . ' (' . $default_currency->code . '</td>
                    ...
                    <?php if ($inv->order_discount != 0) {
                        echo '<tr>
                        <td style="text-align:right;">'.($inv->order_discount_id ? '<small>('.$inv->order_discount_id.')</small> ' : '') . $this->sma->formatMoney($return_purchase ? ($inv->order_discount+$return_purchase->order_discount) : $inv->order_discount) . '</td>
                        <td colspan="' . $col . '" style="text-align:left;">' . '('.$default_currency->code. ')'.' ' .  lang("order_discount") . '</td>
                    ...
                    <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                        echo '<tr>
                        <td style="text-align:right;">' . $this->sma->formatMoney($return_purchase ? ($inv->order_tax+$return_purchase->order_tax) : $inv->order_tax) . '</td>
                        <td colspan="' . $col . '" style="text-align:left;">' . '('.$default_currency->code. ')'.' ' . lang("order_tax") . '</td>
                    ...
                    <?php if ($inv->shipping != 0) {
                        echo '<tr>
                        <td style="text-align:right;">' . $this->sma->formatMoney($inv->shipping) . '</td>
                        <td colspan="' . $col . '" style="text-align:left;">' . '('.$default_currency->code. ')'.' ' . lang("shipping") . '</td>
                    ...
                    <tr>
                        ...
                        <td colspan="<?= $col; ?>" style="text-align:left; font-weight:bold;"> (<?= $default_currency->code; ?>) <?= lang("total_amount"); ?>
                    ...
                    <tr>
                        ...
                        <td colspan="<?= $col; ?>" style="text-align:left; font-weight:bold;">(<?= $default_currency->code; ?>)  <?= lang("balance"); ?> </td>
            ...
            <div class="row">
                ...
                <div class="col-xs-4 col-xs-offset-1 pull-right">
                    <p style="text-align: right"> <?=$created_by->first_name . ' ' . $created_by->last_name;?> : <?=lang("order_by");?> </p>
