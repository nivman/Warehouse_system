    ...
    public function combine_pdf($quotes_id)
    {
        ...
        foreach ($quotes_id as $quote_id) {
            ...
            if($this->Settings->user_language=="hebrew"){
                $html_data_quotes = $this->load->view($this->theme . 'quotes/pdf_rtl', $this->data, true);
            }else{
                $html_data_quotes = $this->load->view($this->theme . 'quotes/pdf', $this->data, true);
            }
            $html[] = array(
                'content' => $html_data_quotes,
                'footer' => '',
            );
        }
...
//                }
               ...
               // $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
                ...
                $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units);
                $c++;
            }
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    if($this->Settings->user_language=="hebrew" &&  $this->input->post('form_action') == 'export_pdf'){
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('B1', lang('total'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        if($this->Settings->user_language=="hebrew" &&  $this->input->post('form_action') == 'export_pdf'){
                            ...
                            $this->excel->getActiveSheet()->SetCellValue('F' . $row, $this->sma->hrld($qu->date));
                            $this->excel->getActiveSheet()->SetCellValue('E' . $row, $qu->reference_no);
                            $this->excel->getActiveSheet()->SetCellValue('D' . $row, $qu->biller);
                            $this->excel->getActiveSheet()->SetCellValue('C' . $row, $qu->customer);
                            $this->excel->getActiveSheet()->SetCellValue('B' . $row, $qu->total);
                            $this->excel->getActiveSheet()->SetCellValue('A' . $row, $qu->status);
                        }else{
                            $qu = $this->quotes_model->getQuoteByID($id);
                    ...
                    $this->excel->getActiveSheet()->getColumnDimension('C')->setWidth(strlen($qu->customer)+10);
                    $this->excel->getActiveSheet()->getColumnDimension('E')->setWidth(strlen($qu->reference_no)+10);
