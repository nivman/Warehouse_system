...
class Auth_app extends MY_Controller
{
    ...
    function check_login_details($conn){
        ...
        if($this->auth_app_model->login($username,$password)){
            ...
            $this->data['user_data'] = $this->auth_app_model->getUserData($username);
            echo json_encode($this->data['user_data'] );
    ...
    function login_app($m = NULL)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->ion_auth->login("owner@tecdiary.com", "12345678", $remember)) {
            ...
            $this->load->library('form_validation');
            $this->load->model('db_model');
            print_r($this->load->model('db_model'));
            // $this->load->view($this->theme . 'auth/login', $this->data);
        }
    }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    {
                        $this->excel->getActiveSheet()->SetCellValue('F1', lang('first_name'));
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('B1', lang('group'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        if($this->Settings->user_language=="hebrew" &&  $this->input->post('form_action') == 'export_pdf')
                        {
                            ...
                            $this->excel->getActiveSheet()->SetCellValue('F' . $row, $user->first_name);
                            $this->excel->getActiveSheet()->SetCellValue('E' . $row, $user->last_name);
                            $this->excel->getActiveSheet()->SetCellValue('D' . $row, $user->email);
                            $this->excel->getActiveSheet()->SetCellValue('C' . $row, $user->company);
                            $this->excel->getActiveSheet()->SetCellValue('B' . $row, $user->group);
                            $this->excel->getActiveSheet()->SetCellValue('A' . $row, $user->status);
                        }else{
                            $user = $this->site->getUser($id);
