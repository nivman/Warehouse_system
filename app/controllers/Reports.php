...
class Reports extends MY_Controller
{
    ...
    function warehouse_stock($warehouse = NULL,$start_date = NULL, $end_date=NULL)
    {
        echo $warehouse;
        ...
        foreach ($warehouses as $warehouse_data) {
            $total_purchases = $this->reports_model->getTotalPurchases($start, $end, $warehouse_data->id);
            $total_sales = $this->reports_model->getTotalSales($start, $end, $warehouse_data->id);
            $total_expenses = $this->reports_model->getTotalExpenses($start, $end, $warehouse_data->id);
    ...
    function warehouseProductTable($warehouse_id = NULL){

        $this->load->library('datatables');
        if ($warehouse_id) {
            $this->datatables
                ->select('image, code, name, wp.quantity,wp.rack')
                ->from('products')
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('alert_quantity'));
                } else {
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->quantity);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->alert_quantity);
                    } else {
    ...
    function comparison()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('comparison_between_products')));
        $meta = array('page_title' => lang('comparison_between_products'), 'bc' => $bc);
        $this->page_construct('reports/comparison', $meta, $this->data);
    }

    function comparison_categories()
    {
        $this->sma->checkPermissions();
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['categories'] = $this->site->getAllCategories();
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('comparison_between_categories')));
        $meta = array('page_title' => lang('comparison_between_categories'), 'bc' => $bc);
        $this->page_construct('reports/comparison_categories', $meta, $this->data);
    }

    function products_and_categories_pie()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('products_and_categories_pie')));
        $meta = array('page_title' => lang('products_and_categories_pie'), 'bc' => $bc);
        $this->page_construct('reports/products_and_categories_pie', $meta, $this->data);
    }
    ...
    function getProductsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->SetCellValue('I1', lang('product_code'));
                    $this->excel->getActiveSheet()->SetCellValue('H1', lang('product_name'));
                    $this->excel->getActiveSheet()->SetCellValue('G1', lang('purchased'));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('sold_amount'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('profit_loss'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('stock_in_hand'));
                } else {
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('product_code'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('product_name'));
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        $this->excel->getActiveSheet()->SetCellValue('I' . $row, $data_row->code);
                        $this->excel->getActiveSheet()->SetCellValue('H' . $row, $data_row->name);
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->PurchasedQty);
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->TotalSales);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->Profit);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->TotalBalance);
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->getStyle("G" . $row . ":A" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
                    $this->excel->getActiveSheet()->SetCellValue('G' . $row, $pQty);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $pl);
                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $bAmt);
                } else {
    ...
    function getCategoriesToComparison()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('categories') . ".code, " . $this->db->dbprefix('categories') . ".name,
                    ...
                    (SUM( COALESCE( PSales.totalSale, 0 ) )- SUM( COALESCE( PCosts.totalPurchase, 0 ) ) ) as Profit, " . $this->db->dbprefix('categories') . ".id")
            ->from('categories')
    ...
    function getProductsDataToPieChart()
    {
        $this->sma->checkPermissions('products', TRUE);
        $start_date = $this->input->get('start_date') ? $this->input->get('start_date') : NULL;
        $end_date = $this->input->get('end_date') ? $this->input->get('end_date') : NULL;

        $pp = "( SELECT product_id, SUM( unit_cost * quantity_balance ) balanceValue,
        ...
        $sp = "( SELECT si.product_id, SUM( si.subtotal ) totalSale from " . $this->db->dbprefix('sales') . " s JOIN " . $this->db->dbprefix('sale_items') . " si on s.id = si.sale_id ";
        if ($start_date) {
            $pp .= " WHERE ";
            $sp .= " WHERE ";
            if ($start_date) {
        ...
        $this->datatables
            ->select($this->db->dbprefix('products') . ".code, " . $this->db->dbprefix('products') . ".name,
				...
				COALESCE( PSales.totalSale, 0 ) as sold,
                (COALESCE( PSales.totalSale, 0 ) - COALESCE( PCosts.totalPurchase, 0 )) as Profit,
				COALESCE( PCosts.balanceValue, 0 ) as balance, {$this->db->dbprefix('products')}.id as id", FALSE)
            ->from('products')
            ->join($sp, 'products.id = PSales.product_id', 'left')
            ->join($pp, 'products.id = PCosts.product_id', 'left')
            ->group_by('products.code, PSales.totalSale, PCosts.totalPurchase, PCosts.balanceValue');
        echo $this->datatables->generate();
    }


    function getCategoriesReportForPie(){
        $this->sma->checkPermissions('products', TRUE);
        $warehouse = $this->input->get('warehouse') ? $this->input->get('warehouse') : NULL;
        $category = $this->input->get('category') ? $this->input->get('category') : NULL;
        $start_date = $this->input->get('start_date') ? $this->input->get('start_date') : NULL;
    ...
    function getCategoriesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->SetCellValue('G1', lang('category_code'));
                    $this->excel->getActiveSheet()->SetCellValue('F1', lang('category_name'));
                    $this->excel->getActiveSheet()->SetCellValue('E1', lang('purchased'));
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('sold'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('purchased_amount'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('sold_amount'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('profit_loss'));
                } else {
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->code);
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->SoldQty);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->TotalPurchase);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->TotalSales);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $profit);
                    } else {
                        $profit = $data_row->TotalSales - $data_row->TotalPurchase;
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->code);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->name);
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->getStyle("E" . $row . ":A" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $pQty);
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $sQty);
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $pAmt);
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $sAmt);
                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $pl);
                    $this->excel->getDefaultStyle()->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                } else {
    ...
    function getBrandsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->SetCellValue('F1', lang('brands'));
                    $this->excel->getActiveSheet()->SetCellValue('E1', lang('purchased'));
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('sold'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('purchased_amount'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('sold_amount'));
    ...
    function daily_sales($warehouse_id = NULL, $year = NULL, $month = NULL, $pdf = NULL, $user_id = NULL)
    {
        ...
        if ($this->Settings->user_language == "hebrew") {
            $calendar_direction = "direction:rtl";
            $daily_sale_direction = "style='text-align: right;'";
        } else {
            $calendar_direction = "direction:ltr";
            $daily_sale_direction = "style='text-align: left;'";
        }
        $config['template'] = '{table_open}<div class="table-responsive" style="' . $calendar_direction . '"><table border="0" cellpadding="0" cellspacing="0" class="table table-bordered dfTable">{/table_open}
        ...
        if (!empty($sales)) {
            foreach ($sales as $sale) {
                $daily_sale[$sale->date] = "<table class='table table-bordered table-hover table-striped table-condensed data' style='margin:0;'><tr><td $daily_sale_direction>" . lang("discount") . "</td><td $daily_sale_direction>" . $this->sma->formatMoney($sale->discount) . "</td></tr><tr><td $daily_sale_direction>" . lang("shipping") . "</td><td $daily_sale_direction>" . $this->sma->formatMoney($sale->shipping) . "</td></tr><tr><td $daily_sale_direction>" . lang("product_tax") . "</td><td $daily_sale_direction>" . $this->sma->formatMoney($sale->tax1) . "</td></tr><tr><td $daily_sale_direction>" . lang("order_tax") . "</td><td $daily_sale_direction>" . $this->sma->formatMoney($sale->tax2) . "</td></tr><tr><td $daily_sale_direction>" . lang("total") . "</td><td $daily_sale_direction>" . $this->sma->formatMoney($sale->total) . "</td></tr></table>";
            }
        } else {
            $daily_sale = array();
        }
        ...
        if ($pdf) {
            ...
            $html = $this->load->view($this->theme . 'reports/daily_for_pdf_export', $this->data, true);
    ...
    function monthly_sales($warehouse_id = NULL, $year = NULL, $pdf = NULL, $user_id = NULL)
    {
        ...
        if ($pdf) {
            if ($this->Settings->user_language == "hebrew") {
                $html = $this->load->view($this->theme . 'reports/monthly_rtl_pdf_export', $this->data, true);
            } else {
    ...
    function getSalesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->SetCellValue('I1', lang('date'));
                    $this->excel->getActiveSheet()->SetCellValue('H1', lang('reference_no'));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('balance'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('payment_status'));
                } else {
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        $this->excel->getActiveSheet()->SetCellValue('I' . $row, $this->sma->hrld($data_row->date));
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, ($data_row->grand_total - $data_row->paid));
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, lang($data_row->payment_status));
                    } else {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->getStyle("D" . $row . ":B" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $total);
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $paid);
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $balance);
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->paid);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, ($data_row->grand_total - $data_row->paid));
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->status);
                    } else {
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($data_row->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->reference_no);
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('F1', lang('payment_reference'));
                    $this->excel->getActiveSheet()->SetCellValue('E1', lang('sale_reference'));
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('purchase_reference'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('paid_by'));
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $this->sma->hrld($data_row->date));
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->payment_ref);
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, lang($data_row->paid_by));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->amount);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->type);
                    } else {
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($data_row->date));
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->getStyle("B" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
    ...
    function getCustomers($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->SetCellValue('H1', lang('company'));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('paid'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('balance'));
                } else {
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        $this->excel->getActiveSheet()->SetCellValue('H' . $row, $data_row->company);
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->name);
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->phone);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->email);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->total);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $this->sma->formatMoney($data_row->total_amount));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $this->sma->formatMoney($data_row->paid));
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->formatMoney($data_row->balance));
                        $this->excel->getDefaultStyle()->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                    } else {
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('F1', lang('phone'));
                    $this->excel->getActiveSheet()->SetCellValue('E1', lang('email'));
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('total_purchases'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('total_amount'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('paid'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('balance'));
                } else {
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->email);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->total);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->total_amount);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->paid);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->balance);
                    } else {
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->company);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->name);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->phone);
    ...
    function warehouseBalance($start_date = NULL, $end_date = NULL){
      ...
      //  $end_date =$_POST['end_date'];
        ...
        echo json_encode($warehouses_report);
    ...
    function getRrgisterlogs($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->SetCellValue('K1', lang('open_time'));
                    $this->excel->getActiveSheet()->SetCellValue('J1', lang('close_time'));
                    $this->excel->getActiveSheet()->SetCellValue('I1', lang('user'));
                    $this->excel->getActiveSheet()->SetCellValue('H1', lang('cash_in_hand'));
                    $this->excel->getActiveSheet()->SetCellValue('G1', lang('cc_slips'));
                    $this->excel->getActiveSheet()->SetCellValue('F1', lang('cheques'));
                    $this->excel->getActiveSheet()->SetCellValue('E1', lang('total_cash'));
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('cc_slips_submitted'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('cheques_submitted'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('total_cash_submitted'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('note'));
                } else {
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        $this->excel->getActiveSheet()->SetCellValue('K' . $row, $this->sma->hrld($data_row->date));
                        $this->excel->getActiveSheet()->SetCellValue('J' . $row, $data_row->closed_at);
                        $this->excel->getActiveSheet()->SetCellValue('I' . $row, $data_row->user);
                        $this->excel->getActiveSheet()->SetCellValue('H' . $row, $data_row->cash_in_hand);
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->total_cc_slips);
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->total_cc_slips_submitted);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->total_cheques_submitted);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->total_cash_submitted);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->note);
                        if ($data_row->total_cash_submitted < $data_row->total_cash || $data_row->total_cheques_submitted < $data_row->total_cheques || $data_row->total_cc_slips_submitted < $data_row->total_cc_slips) {
                            $this->excel->getActiveSheet()->getStyle('K' . $row . ':A' . $row)->applyFromArray(
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('created_by'));
                } else {
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('reference_no'));
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $this->sma->hrld($data_row->date));
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->reference);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->category);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->amount);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->note);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->created_by);
                    } else {
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($data_row->date));
                ...
                if ($this->Settings->user_language == "hebrew") {
                    $this->excel->getActiveSheet()->getStyle("C" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
    ...
    function daily_purchases($warehouse_id = NULL, $year = NULL, $month = NULL, $pdf = NULL, $user_id = NULL)
    {
        ...
        if ($this->Settings->user_language == "hebrew") {
            ...
            $daily_purchases_direction = "style='text-align: right;'";
        } else {

            $daily_purchases_direction = "style='text-align: left;'";
        }
        if (!empty($purchases)) {
            foreach ($purchases as $purchase) {
                $daily_purchase[$purchase->date] = "<table class='table table-bordered table-hover table-striped table-condensed data' style='margin:0;'><tr><td $daily_purchases_direction>" . lang("discount") . "</td><td $daily_purchases_direction>" . $this->sma->formatMoney($purchase->discount) . "</td></tr><tr><td $daily_purchases_direction>" . lang("shipping") . "</td><td $daily_purchases_direction>" . $this->sma->formatMoney($purchase->shipping) . "</td></tr><tr><td $daily_purchases_direction>" . lang("product_tax") . "</td><td $daily_purchases_direction>" . $this->sma->formatMoney($purchase->tax1) . "</td></tr><tr><td $daily_purchases_direction>" . lang("order_tax") . "</td><td>" . $this->sma->formatMoney($purchase->tax2) . "</td></tr><tr><td>" . lang("total") . "</td><td>" . $this->sma->formatMoney($purchase->total) . "</td></tr></table>";
            }
        } else {
        ...
        if ($pdf) {
            if ($this->Settings->user_language == "hebrew") {
                $html = $this->load->view($this->theme . 'reports/daily_purchases_rtl', $this->data, true);
            } else {
                $html = $this->load->view($this->theme . 'reports/daily_purchases', $this->data, true);
            }
    ...
    function monthly_purchases($warehouse_id = NULL, $year = NULL, $pdf = NULL, $user_id = NULL)
    {
        ...
        if ($pdf) {
            if ($this->Settings->user_language == "hebrew") {
                $html = $this->load->view($this->theme . 'reports/monthly_purchases_rtl', $this->data, true);
            } else {
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                if ($this->Settings->user_language == "hebrew") {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('note'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('products'));
                } else {
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('reference_no'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('warehouse'));
                ...
                foreach ($data as $data_row) {
                    if ($this->Settings->user_language == "hebrew") {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->wh_name);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->created_by);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $this->sma->decode_html($data_row->note));
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->iname);
                    } else {
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($data_row->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->reference_no);
