...
class Products_model_ap extends CI_Model
{
    ...
    public function joinProductAndCategory()
    {
        $data = [];
        $this->db->select($this->db->dbprefix('products') . '.*, ' . $this->db->dbprefix('units') . '.name as units');
...
//        }
    ...
    public function getPhotoById($id)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            $row_decode = json_decode($row_encode, true);
            $key = "image";
            $image = $row_decode[$key];
            return $image;
        }
        return FALSE;
    }
    ...
    public function addAdjustment($data, $products, $adjustment)
    {
    ...
    public function updateAdjustment($id, $data, $products, $adjustment)
    {
